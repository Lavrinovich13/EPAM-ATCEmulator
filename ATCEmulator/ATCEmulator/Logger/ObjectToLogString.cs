...
namespace ATCEmulator
{
    public static class ObjectToLogString
    {
        ...
        public static string ToLogString(Respond respond)
        {
            return String.Format("Respond state is {2} on call from {0} to {1}",
                respond.IncomingRequest.SourceNumber.GetValue, respond.IncomingRequest.TargetNumber.GetValue, respond.State);
        }
        ...
        protected static string ArrayToString(PhoneNumber[] array)
        {
        ...
        {
            return String.Format("Contract: Date - {0}, Number - {1}, /n/t {2} ",
                contract.Date.ToShortDateString(), contract.Number.GetValue, contract.TariffPlan);
        }
    }
}
